...
import { getInfoTheater } from "slices/info-theater";
import { getTheaterList } from "slices/list-theater";
import { getInfoCinema } from "slices/menuCinema";
...
const MenuCinema: React.FC = () => {
  const listCinema = useSelector((state: RootState) => state.info_cinema);
  const listTheater = useSelector((state: RootState) => state.list_theater);
  const listInfoTheater = useSelector(
    (state: RootState) => state.list_info_theater
  );
  ...
  return (
    <div className="text-center my-10 md:container md:mx-auto">
      <h1 className="mb-10 text-3xl font-semibold">
        Lịch Chiếu Và Thông Tin Tất Cả Các Rạp
      </h1>
      ...
      <div
        className="flex justify-start text-left md:container md:mx-auto boder"
              ...
              return (
                ...
                cumrap.danhSachPhim.map((movive, index) => {
                  return (
                    <div
                      key={index}
                      className="flex flex-row border mb-4 p-8 space-y-4 "
                    >
                      ...
                      <div className="pl-6">
                        <div className="flex gap-2">
                          <span className="bg-red-500 w-9 text-center rounded-md">
                            C18
                          </span>
                        ...
                        <div className=" pt-5 flex flex-wrap gap-4">
                          {movive.lstLichChieuTheoPhim.map(
                            (lichchieu, index) => {
                              return (
                                index < Config.PER_PAGE_TIME && (
                                  <div
                                    ...
                                    onClick={() =>
                                      goToBuyTicket(lichchieu.maLichChieu)
                                    }
                                  >
                                    <div>
